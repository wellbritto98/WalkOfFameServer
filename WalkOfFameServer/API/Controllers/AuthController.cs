...
namespace WalkOfFameServer.API.Controllers
{
    public class AuthController : CoreController
    {
        ...
        public AuthController(UserService service, UtilsService utils, IMapper mapper) : base(service)
        {
            _utils = utils;
            _mapper = mapper;
        }
        ...
        {
            var checkEmail = await _userService.GetByEmail(request.Email);

            if (checkEmail != null) return Conflict(new { message = "E-mail address already taken!" });

            var checkName = await _userService.GetByUserName(request.UserName);
